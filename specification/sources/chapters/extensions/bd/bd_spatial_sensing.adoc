...
permissions to use their capabilities on AOSP-based platforms.
...
.Note
====
A runtime implementing an earlier version of this extension will not
recognize elink:XrSemanticLabelBD values introduced in a later extension
version and therefore is unable to filter on such labels.
If a label introduced in a later version of the extension than the runtime
reports is passed in pname:labels, no corresponding data will be returned.
====
...
<<fundamentals-buffer-size-parameters,two-call idiom>>, with entity ID,
UUID, and last update times for each entity in the snapshot.
...
[open,refpage='XrSpatialEntityComponentDataSphereBD',type='structs',desc='Spatial entity sphere component data.',xrefs='']
--
The slink:XrSpatialEntityComponentDataSphereBD structure is defined as:
include::{generated}/api/structs/XrSpatialEntityComponentDataSphereBD.adoc[]
...
* pname:next is code:NULL or a pointer to the next structure in a structure
  chain.
* pname:sphere is an slink:XrSpheref defining the center position and
  orientation and the radius of a sphere bounds relative to the spatial
  entity.
****

The slink:XrSpatialEntityComponentDataSphereBD is an output struct for
getting the sphere component data from an entity in the snapshot.
This corresponds to an elink:XrSpatialEntityComponentTypeBD value of
ename:XR_SPATIAL_ENTITY_COMPONENT_TYPE_SPHERE_BD.

include::{generated}/validity/structs/XrSpatialEntityComponentDataSphereBD.adoc[]
--
...
XrFutureEXT providerCreateFuture {XR_NULL_FUTURE_EXT};

XrSenseDataProviderCreateInfoFooBD providerCreateInfoFoo {
    .type = XR_TYPE_SENSE_DATA_PROVIDER_CREATE_INFO_FOO_BD,
};
XrSenseDataProviderCreateInfoBD providerCreateInfo {
    .type = XR_TYPE_SENSE_DATA_PROVIDER_CREATE_INFO_BD,
    .next = &providerCreateInfoFoo,
};
...
// Start foo provider
XrFutureEXT providerStartFuture {XR_NULL_FUTURE_EXT};
...
auto queryFooSpatialEntities = [&]()
{
    ...
    XrFutureEXT queryFuture {XR_NULL_FUTURE_EXT};
    ...
    XrQueriedSenseDataGetInfoBD queriedDataGetInfo {
        ...
        for (int i = 0; i < queriedSenseData.stateCountOutput; ++i)
        {
            ...
            if (componentTypeCountOutput > 0)
            {
                ...
                for (int j = 0; j < componentTypeCountOutput; ++j)
                {
                    ...
                    {
                        XrSpatialEntityComponentGetInfoBD componentBarGetInfo {
                            .type = XR_TYPE_SPATIAL_ENTITY_COMPONENT_GET_INFO_BD,
                            .next = nullptr,
                            .entityId = entityId,
                            .componentType = XR_SPATIAL_ENTITY_COMPONENT_TYPE_BAR_BD,
                        };
                        XrSpatialEntityComponentDataBarBD componentBarData {
                            .type = XR_TYPE_SPATIAL_ENTITY_COMPONENT_DATA_BAR_BD,
                        };
...
if (queriedSenseData.stateCountOutput > 0)
{
    ...
    for (int i = 0; i < queriedSenseData.stateCountOutput; ++i)
    {
        ...
        if (componentTypeCountOutput > 0)
        {
            ...
            {
                XrSpatialEntityLocationGetInfoBD locationGetInfo {
                ...
                XrSpatialEntityComponentGetInfoBD componentGetInfo {
                    .type = XR_TYPE_SPATIAL_ENTITY_COMPONENT_GET_INFO_BD,
                    .next = &locationGetInfo,
                    .entityId = entityId,
                    .componentType = XR_SPATIAL_ENTITY_COMPONENT_TYPE_LOCATION_BD,
                };

                XrSpatialEntityComponentDataLocationBD componentLocationData {
            ...
            if(hasComponentType(entityComponentTypes, XR_SPATIAL_ENTITY_COMPONENT_TYPE_BOUNDING_BOX_2D_BD))
            {
                XrSpatialEntityComponentGetInfoBD componentGetInfo {
                    ...
                    .next = nullptr,
                    .entityId = entityId,
                    .componentType = XR_SPATIAL_ENTITY_COMPONENT_TYPE_BOUNDING_BOX_2D_BD,
                };

                XrSpatialEntityComponentDataBoundingBox2DBD componentBoundingBox2DData {
            ...
            if(hasComponentType(entityComponentTypes, XR_SPATIAL_ENTITY_COMPONENT_TYPE_TRIANGLE_MESH_BD))
            {
                XrSpatialEntityComponentGetInfoBD componentGetInfo {
                    ...
                    .next = nullptr,
                    .entityId = entityId,
                    .componentType = XR_SPATIAL_ENTITY_COMPONENT_TYPE_TRIANGLE_MESH_BD,
                };

                XrSpatialEntityComponentDataTriangleMeshBD componentTriangleMeshData {
...
* Revision 2, 2025-08-05 (Zijian Wang)

** Add the sphere component data struct

* Revision 3, 2026-05-29 (Xu Yang)

** Add new semantic label definitions for keyboard, mouse, and laptop

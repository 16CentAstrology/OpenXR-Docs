...
an object as "started" when the basetype:XrFutureEXT is in ready state,
...
XrFutureEXT futureFooObject {XR_NULL_FUTURE_EXT};
XrResult result;

XrFooObjectCreateInfo createInfo{
  .type = XR_TYPE_FOO_OBJECT_CREATE_INFO,
};
...
if (futureReady) {
  XrFooObjectCreateCompletionEXT completion{
    .type = XR_TYPE_FOO_OBJECT_CREATE_COMPLETION,
  };
...
XrFutureEXT futureFooObjects {XR_NULL_FUTURE_EXT};
XrResult result;

XrFooObjectCreateInfo createInfo{
  .type = XR_TYPE_FOO_OBJECTS_CREATE_INFO,
};
...
if (futureReady) {
  XrFooObjectsCreateCompletionEXT completion{
    .type = XR_TYPE_FOO_OBJECTS_CREATE_COMPLETION
  };
  ...
  if (completion.elementCapacityOutput != 0) {
    ...
    completion.elementCapacityInput = static_cast<uint32_t>(floatValues.size());
...
class MyGame {
  void OnSlowFooRequest() {
    if (m_slowFooFuture == XR_NULL_FUTURE_EXT) {
      ...
      XrSlowFooInfoEXT fooInfo{
        .type = XR_TYPE_SLOW_FOO_INFO_EXT,
      };
  ...
  void OnGameTickOrSomeOtherReoccurringFunction() {
    ...
    if (pollResult.state == XR_FUTURE_STATE_READY_EXT) {
      ...
      XrSlowFooCompletionEXT completion{
        .type = XR_TYPE_SLOW_FOO_COMPLETION_EXT,
      };
      XrResult result = xrSlowFooCompleteEXT(session, m_slowFooFuture, &completion);

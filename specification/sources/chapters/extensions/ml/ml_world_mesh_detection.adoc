...
class MeshDetector {
  private:
    ...
    std::array<XrWorldMeshBufferML, 2> m_WorldMeshBuffers{{
      {
    ...
    bool StartInfoQuery(XrTime displayTime) {
      XrWorldMeshStateRequestInfoML requestInfo {
        ...
        .boundingBoxExtents = {10.0f, 10.0f, 10.0f},
      };
    ...
    bool CompleteInfoQuery() {
      ...
      if (completion.meshBlockStateCountOutput>m_MaxBlockCounts[m_QueryBuffer]) {
        ...
        XrWorldMeshBufferSizeML bufferSize {
    ...
    bool StartMeshQuery() {
      std::vector<XrWorldMeshBlockRequestML> blockRequests(m_MeshBlocks.size());
      for (size_t i = 0; i< m_MeshBlocks.size();i++) {
        blockRequests[i].type = XR_TYPE_WORLD_MESH_BLOCK_REQUEST_ML;
        blockRequests[i].next = nullptr;
      ...
      XrWorldMeshGetInfoML getInfo {
        ...
        .flags = 0,
        .fillHoleLength = 0.5f,
        .disconnectedComponentArea = 1.0f,
        .blockCount = static_cast<uint32_t>(blockRequests.size()),
        .blocks = blockRequests.data(),
      };
    ...
    bool CompleteMeshQuery(XrTime displayTime) {
      ...
      XrWorldMeshRequestCompletionML completion {
        ...
        .blockCount = static_cast<uint32_t>(m_WorldMeshBlocks.size()),
        .blocks = m_WorldMeshBlocks.data(),
      };
      XrWorldMeshRequestCompletionInfoML completionInfo {
        .type = XR_TYPE_WORLD_MESH_REQUEST_COMPLETION_INFO_ML,
        .meshSpace = m_LocalSpace,
        .meshSpaceLocateTime = displayTime,
      };

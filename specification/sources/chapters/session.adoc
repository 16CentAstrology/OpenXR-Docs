...
flink:xrEndSession.
ifdef::XR_EXT_spatial_container[]
When the apiext:XR_EXT_spatial_container extension is available and
<<ext_spatial_container-changes_to_session_state,spatial container support
is enabled>>, a session is considered running while it is in the state
ename:XR_SESSION_STATE_IDLE.
endif::XR_EXT_spatial_container[]
...
When the apiext:XR_EXT_spatial_container extension is available and
<<ext_spatial_container-changes_to_session_state,spatial container support
is enabled>>, a session is considered not running while it is not in the
state ename:XR_SESSION_STATE_IDLE.
endif::XR_EXT_spatial_container[]
